...
inline void hessianResponseImpl(const cv::Mat &inputImage,
                                       cv::Mat &outputImage) {
...
inline void hessianResponse(const cv::Mat &inputImage, cv::Mat &outputImage) {
...
inline int findRoot(std::vector<int> &cluster_ids, int id) {
  if (id == -1)
    return id;
  // check root
...
inline bool point_comparator(const cv::Point2i &a, const cv::Point2i &b) {
  if (a.y != b.y)
    return a.y < b.y;
  else
    return (a.x < b.x);
