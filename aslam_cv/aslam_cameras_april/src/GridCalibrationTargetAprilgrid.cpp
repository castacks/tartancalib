...
bool GridCalibrationTargetAprilgrid::computeObservationDeltille(
    const cv::Mat & image, Eigen::MatrixXd & outImagePoints,
  ...
  deltilleDetector.run(image, corners, boards, debug ? &debug_image : nullptr);

  assert(boards.size() == 1);
  orp::calibration::BoardObservation obs = boards.at(0);
  const cv::Mat checkerboard = obs.board;
  const std::vector<cv::Point2f> detections = obs.corner_locations; // analogous to AprilTag detections normally.

  // DEBUG 1: PRINTING
  // for (int r = 0; r < checkerboard.rows; ++r) {
  //   for (int c = 0; c < checkerboard.cols; ++c) {
  //     std::cout << "Row:" << r << " Column:" << c << "=(" << detections[checkerboard.at<int>(r, c)].x << "," << detections[checkerboard.at<int>(r, c)].y << ")" << std::endl;
  //     bool validity = (detections[checkerboard.at<int>(r, c)].x > 0) && (detections[checkerboard.at<int>(r, c)].y > 0);
  //     // std::cout << "Valid point:" << validity << std::endl;
  //   }  
  // }

  // DEBUG 2: VISUALIZING
  // std::string fn = GetCurrentTimeForFileName();
  ...
  for (int r = 0; r < checkerboard.rows; ++r) {
    for (int c = 0; c < checkerboard.cols; ++c) {
      int cornerId = checkerboard.at<int>(r, c);
      if ((detections[cornerId].x > 0) && (detections[cornerId].y > 0)) {
        valid_tags++;
      }
    }  
  }
  if (valid_tags < _options.minTagsForValidObs) {
    success = false;
  ...
  // TODO: Implement extraction video with Deltille.
  // if (_options.showExtractionVideo) {
  //   //image with refined (blue) and raw corners (red)
  //   cv::Mat imageCopy1 = image.clone();
  //   cv::cvtColor(imageCopy1, imageCopy1, cv::COLOR_GRAY2RGB);
  ...
  for (int r = 0; r < checkerboard.rows; ++r) {
    for (int c = 0; c < checkerboard.cols; ++c) {
      int cornerId = checkerboard.at<int>(r, c);
      unsigned int insertionIndex = r * checkerboard.cols + c;
      outImagePoints.row(insertionIndex) = Eigen::Matrix<double, 1, 2>(detections[cornerId].x, detections[cornerId].y);
      if (detections[cornerId].x > 0 && detections[cornerId].y > 0) {
        outCornerObserved[insertionIndex] = true;
      }
      else {
        outCornerObserved[insertionIndex] = false;
      }
    }  
  }
